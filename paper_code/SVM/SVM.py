...
# Scale the features
scaler = StandardScaler().fit(X['train'].cpu().numpy())
X_scaled = {k: scaler.transform(v.cpu().numpy()) for k, v in X.items()}

# Initialize the SVM model
svm_model = SVC(probability=True, kernel='rbf', class_weight='balanced')

# Train the model
svm_model.fit(X_scaled['train'], y['train'].cpu().numpy())

# Predict on validation and test sets
val_preds = svm_model.predict_proba(X_scaled['val'])
test_preds = svm_model.predict_proba(X_scaled['test'])
